var WASTE = [];
WASTE.length = 8;
WASTE[0] = [
  ['00', 39.2431787,-94.7370019,'1'],
  ['01', 39.2898592,-94.6863722,'1'],
  ['02', 39.2710242,-94.6678453,'1'],
  ['03', 39.2606268,-94.6623577,'1'],
  ['04', 39.263029,-94.7242035,'1'],
  ['05', 39.2727505,-94.6988655,'1'],
  ['06', 39.2789957,-94.6276975,'1'],
  ['07', 39.2351913,-94.7037787,'1'],
  ['08', 39.2298856,-94.6880157,'1'],
  ['09', 39.2305248,-94.6433863,'1'],
  ['10', 39.2382414,-94.6403928,'1'],
  ['11', 39.240959,-94.6552524,'1'],
  ['12', 39.2190493,-94.6909831,'1'],
  ['13', 39.2189333,-94.6482306,'1'],
  ['14', 39.2911898,-94.8058151,'1'],
];
WASTE[1] = [
  ['15', 39.2754468,-94.5993321,'2'],
  ['16', 39.2845263,-94.5833559,'2'],
  ['17', 39.2888818,-94.5630145,'2'],
  ['18', 39.2591714,-94.6400579,'2'],
  ['19', 39.2675453,-94.6154872,'2'],
  ['20', 39.265306,-94.5847927,'2'],
  ['21', 39.2632678,-94.5858203,'2'],
  ['22', 39.2574029,-94.5882033,'2'],
  ['23', 39.2518442,-94.586367,'2'],
  ['24', 39.2517741,-94.5729821,'2'],
  ['25', 39.2539061,-94.5667222,'2'],
  ['26', 39.2243489,-94.5813344,'2'],
  ['27', 39.2247728,-94.5554065,'2'],
  ['28', 39.2178903,-94.5419984,'2'],
  ['29', 39.1872093,-94.544712,'2']
];
WASTE[2] = [
  ['30', 39.1657878,-94.5902334,'3'],
  ['31', 39.2359789,-94.4959507,'3'],
  ['32', 39.3543179,-94.4260174,'3'],
  ['33', 39.3885007,-94.3448454,'3'],
  ['34', 39.4083557,-94.352175,'3'],
  ['35', 39.413057,-94.3715472,'3'],
  ['36', 39.4388803,-94.3393725,'3'],
  ['37', 39.4194278,-94.3163336,'3'],
  ['38', 39.3468861,-94.2767125,'3'],
  ['39', 39.3138709,-94.2923066,'3'],
  ['40', 39.2723572,-94.3552201,'3'],
  ['41', 39.265447,-94.3699433,'3'],
  ['42', 39.2541205,-94.3671224,'3'],
  ['43', 39.2228467,-94.4076589,'3'],
  ['44', 39.2149504,-94.3926815,'3'],
];
WASTE[3] = [
  ['45', 39.1990828,-94.8767134,'4'],
  ['46', 39.1491503,-94.8610692,'4'],
  ['47', 39.1006909,-94.8405625,'4'],
  ['48', 39.0785286,-94.8413468,'4'],
  ['49', 39.0358291,-94.7863306,'4'],
  ['50', 39.0411093,-94.7714265,'4'],
  ['51', 39.0377457,-94.762706,'4'],
  ['52', 39.0629461,-94.7348585,'4'],
  ['53', 39.1051807,-94.7177383,'4'],
  ['54', 39.1038394,-94.6711131,'4'],
  ['55', 39.0814747,-94.6713114,'4'],
  ['56', 39.0816183,-94.6528674,'4'],
  ['57', 39.1008603,-94.6423381,'4'],
  ['58', 39.1252578,-94.6217065,'4'],
  ['59', 39.1236686,-94.6212188,'4'],
];
WASTE[4] = [
  ['60', 39.1000706,-94.5940876,'5'],
  ['61', 39.1047656,-94.587772,'5'],
  ['62', 39.1031992,-94.5828907,'5'],
  ['63', 39.1242372,-94.5906439,'5'],
  ['64', 39.1120083,-94.5713383,'5'],
  ['65', 39.1050305,-94.5678021,'5'],
  ['66', 39.0976358,-94.5747986,'5'],
  ['67', 39.0814663,-94.5864272,'5'],
  ['68', 39.0862955,-94.5763322,'5'],
  ['69', 39.0849556,-94.5762812,'5'],
  ['70', 39.0627993,-94.5696039,'5'],
  ['71', 39.0705199,-94.5517173,'5'],
  ['72', 39.0893246,-94.5422069,'5'],
  ['73', 39.1091196,-94.5162243,'5'],
  ['74', 39.0468762,-94.5815972,'5'],
];
WASTE[5] = [
  ['75', 39.1105869,-94.4711014,'6'],
  ['76', 39.0886768,-94.4834321,'6'],
  ['77', 39.0837554,-94.4823369,'6'],
  ['78', 39.0823413,-94.5173375,'6'],
  ['79', 39.0599808,-94.5871244,'6'],
  ['80', 39.0627508,-94.5162774,'6'],
  ['81', 39.111187,-94.5396423,'6'],
  ['82', 39.083904,-94.5000264,'6'],
  ['83', 39.0765059,-94.5137466,'6'],
  ['84', 39.0554949,-94.4835117,'6'],
  ['85', 39.0706166,-94.431093,'6'],
  ['86', 39.0568098,-94.4224853,'6'],
  ['87', 39.0795022,-94.4186981,'6'],
  ['88', 39.1032805,-94.4469273,'6'],
  ['89', 39.0363955,-94.4414704,'6'],
];
WASTE[6] = [
  ['90', 38.9912292,-94.6836133,'7'],
  ['91', 38.9750091,-94.7086072,'7'],
  ['92', 38.977348,-94.7174573,'7'],
  ['93', 38.8931098,-94.7009274,'7'],
  ['94', 38.8873278,-94.7020264,'7'],
  ['95', 38.8846129,-94.700396,'7'],
  ['96', 38.899684,-94.685969,'7'],
  ['97', 38.9088063,-94.6453009,'7'],
  ['98', 38.9062818,-94.6400698,'7'],
  ['99', 38.9404039,-94.7381737,'7'],
  ['100', 38.8491518,-94.6570899,'7'],
  ['101', 39.0015376,-94.5861012,'7'],
  ['102', 38.9928591,-94.5714563,'7'],
  ['103', 38.9818346,-94.5682575,'7'],
  ['104', 38.9656586,-94.6055493,'7'],
];
WASTE[7] = [
  ['105', 39.0188667,-94.6252613,'8'],
  ['106', 39.0010317,-94.5814841,'8'],
  ['107', 38.9866629,-94.5655617,'8'],
  ['108', 38.9827959,-94.5675494,'8'],
  ['109', 38.9610732,-94.5917958,'8'],
  ['110', 38.9431893,-94.4491258,'8'],
  ['111', 38.9844884,-94.5126759,'8'],
  ['112', 38.9980997,-94.4805634,'8'],
  ['113', 38.9836225,-94.492455,'8'],
  ['114', 39.0148518,-94.4254242,'8'],
  ['115', 38.9997563,-94.4216776,'8'],
  ['116', 38.9710594,-94.5574532,'8'],
  ['117', 38.9745248,-94.5987889,'8'],
  ['118', 39.0090782,-94.469559,'8'],
  ['119', 38.981971,-94.5066791,'8'],
];

window.onload = function() {
  if (sessionStorage.getItem('logged_in') !== '1') {
    alert('Please log in first.');
    window.location.href = '../index.html';
  }
  sessionStorage.setItem('z_1', '1');
  sessionStorage.setItem('z_2', '1');
  sessionStorage.setItem('z_3', '1');
  sessionStorage.setItem('z_4', '1');
  sessionStorage.setItem('z_5', '1');
  sessionStorage.setItem('z_6', '1');
  sessionStorage.setItem('z_7', '1');
  sessionStorage.setItem('z_8', '1');
  localStorage.setItem('add_driver', '0');
  var drivers = sessionStorage.getItem('drivers');
  drivers = drivers.split(',');
  var driver_script = '';
  for (var i = 0; i < drivers.length; i++) {
    driver_script += '<tr id = \'driver\'>';
    driver_script += '<td class = \'driver\'>' + drivers[i] + '</td>';
    driver_script += '<td class = \'button_2\' align = \'right\'>' + '<button class = \'blue_button\' onclick = \'chat("' + drivers[i] + '");\'>' + 'Chat' + '</button>' + '<button class = \'small_button\' onclick = \'ddelete("' + drivers[i] + '");\'>' + 'Delete' + '</button>' + '<button class = \'small_button\' onclick = \'location1("' + drivers[i] + '");\'>' + 'Location' + '</button>' + '</td>';
    driver_script += '</tr>';
  }
  $('#driver').replaceWith(driver_script + '<tr><td><br><br></td></tr>');
}

$(
  function() {
    $('#sign_out').click(
      function() {
        sessionStorage.setItem('logged_in', '0');
        window.location.href = '../index.html';
      }
    );
  }
);

$(
  function() {
    $('#add_driver').click(
      function() {
        var win = window.open('Add_Driver.html', 'Add Driver', 'width=400,height=600');
        var timer = setInterval(
          function() {
            if (win.closed) {
              clearInterval(timer);
              if (localStorage.getItem('add_driver') === '1') {
                localStorage.setItem('add_driver', '0');
                var W = [];
                var pri = 1;
                for (var i = 0; i < 8; i++) {
                  for (var j = 0; j < 15; j++) {
                    W.push({
                      'waste_id': parseInt(WASTE[i][j][0]).toString(),
                      'latitude': WASTE[i][j][1],
                      'longtitude': WASTE[i][j][2],
                      'zone_id': (parseInt(WASTE[i][j][3]) - 1).toString(),
                      'priority': pri.toString()
                    });
                    pri++;
                  }
                }
                $.ajax(
                  {
                    url: "https://api.mlab.com/api/1/databases/skydefender/collections/sky?apiKey=L3tVr3ypZdG3UXlyiek9YP3bZqbBQexR",
                    data: JSON.stringify(
                      {
                        "_id": localStorage.getItem('newID'),
                        "password": localStorage.getItem('newPassword'),
                        "email": localStorage.getItem('newEmail'),
                        "admin": "0",
                        "source_list": JSON.parse(sessionStorage.getItem('source')),
                        "Destination": JSON.parse(sessionStorage.getItem('destination')),
                        "waste_list": W,
                        "current_location": {
                          "waste_id": '0'
                        }
                      }
                    ),
                    type: "POST",
                    contentType: "application/json",
                    success: function(result) {
                      var drivers = sessionStorage.getItem('drivers');
                      drivers += ',' + localStorage.getItem('newID');
                      sessionStorage.setItem('drivers', drivers);
                      var D = drivers.split(',');
                      var D1 = [];
                      for (var i = 0; i < D.length; i++) {
                        D1.push(
                          {
                            'id': D[i]
                          }
                        );
                      }
                      $.ajax(
                        {
                          url: "https://api.mlab.com/api/1/databases/skydefender/collections/sky/" + sessionStorage.getItem('id') + "?apiKey=L3tVr3ypZdG3UXlyiek9YP3bZqbBQexR",
                          data: JSON.stringify(
                            {
                              "$set": {
                                "driver_list": D1
                              }
                            }
                          ),
                          type: "PUT",
                          contentType: "application/json",
                          success: function(result) {
                            location.reload();
                          },
                          error: function(error) {
                            alert("Error: cannot register!  Please try later.");
                          }
                        }
                      );
                    },
                    error: function(error) {
                      alert("Error: cannot register!  Please try later.");
                    }
                  }
                );
              }
            }
          }, 500
        );
      }
    );
  }
);

function ddelete(id) {
  var c = window.confirm('The operation is irreversable!  Would you like to continue?');
  if (c) {
    $.ajax(
      {
        url: "https://api.mlab.com/api/1/databases/skydefender/collections/sky/" + id + "?apiKey=L3tVr3ypZdG3UXlyiek9YP3bZqbBQexR",
        type: "DELETE",
        success: function(result) {
          var drivers = sessionStorage.getItem('drivers');
          drivers = drivers.split(',');
          for (var i = 0; i < driver.length; i++) {
            if (drivers[i] === id) {
              drivers[i] = null;
            }
          }
          var D = '';
          var D1 = [];
          for (var i = 0; i < drivers.length; i++) {
            if (drivers[i] !== null) {
              D += (i === 0 ? '' : ',');
              D += drivers[i];
              D1.push(
                {
                  'id': drivers[i]
                }
              );
            }
          }
          console.log(D);
          sessionStorage.setItem('drivers', D);
          $.ajax(
            {
              url: "https://api.mlab.com/api/1/databases/skydefender/collections/sky/" + sessionStorage.getItem('id') + "?apiKey=L3tVr3ypZdG3UXlyiek9YP3bZqbBQexR",
              data: JSON.stringify(
                {
                  "$set": {
                    "driver_list": D1
                  }
                }
              ),
              type: "PUT",
              contentType: "application/json",
              success: function(result) {
                location.reload();
              },
              error: function(error) {
                alert("Error: cannot register!  Please try later.");
              }
            }
          );
        },
        error: function(error) {
          alert("Error: cannot delete!  Please try later.");
        }
      }
    );
  }
}

function chat(you) {
  sessionStorage.setItem('you', you);
  window.open('Chat.html', 'Sky Defender Chatting App', 'width=400,height=600');
}

function initMap() {
  var umkc = {lat: 39.1366392, lng: -94.5229319};
  var map = new google.maps.Map(document.getElementById('map'), {
    zoom: 11,
    center: umkc
  });
  var multiple_markers_01 = [
    ['00', 39.2431787,-94.7370019,'1'],
    ['01', 39.2898592,-94.6863722,'1'],
    ['02', 39.2710242,-94.6678453,'1'],
    ['03', 39.2606268,-94.6623577,'1'],
    ['04', 39.263029,-94.7242035,'1'],
    ['05', 39.2727505,-94.6988655,'1'],
    ['06', 39.2789957,-94.6276975,'1'],
    ['07', 39.2351913,-94.7037787,'1'],
    ['08', 39.2298856,-94.6880157,'1'],
    ['09', 39.2305248,-94.6433863,'1'],
    ['10', 39.2382414,-94.6403928,'1'],
    ['11', 39.240959,-94.6552524,'1'],
    ['12', 39.2190493,-94.6909831,'1'],
    ['13', 39.2189333,-94.6482306,'1'],
    ['14', 39.2911898,-94.8058151,'1'],
  ];
  var multiple_markers_02 = [
    ['15', 39.2754468,-94.5993321,'2'],
    ['16', 39.2845263,-94.5833559,'2'],
    ['17', 39.2888818,-94.5630145,'2'],
    ['18', 39.2591714,-94.6400579,'2'],
    ['19', 39.2675453,-94.6154872,'2'],
    ['20', 39.265306,-94.5847927,'2'],
    ['21', 39.2632678,-94.5858203,'2'],
    ['22', 39.2574029,-94.5882033,'2'],
    ['23', 39.2518442,-94.586367,'2'],
    ['24', 39.2517741,-94.5729821,'2'],
    ['25', 39.2539061,-94.5667222,'2'],
    ['26', 39.2243489,-94.5813344,'2'],
    ['27', 39.2247728,-94.5554065,'2'],
    ['28', 39.2178903,-94.5419984,'2'],
    ['29', 39.1872093,-94.544712,'2']
  ];
  var multiple_markers_03 = [
    ['30', 39.1657878,-94.5902334,'3'],
    ['31', 39.2359789,-94.4959507,'3'],
    ['32', 39.3543179,-94.4260174,'3'],
    ['33', 39.3885007,-94.3448454,'3'],
    ['34', 39.4083557,-94.352175,'3'],
    ['35', 39.413057,-94.3715472,'3'],
    ['36', 39.4388803,-94.3393725,'3'],
    ['37', 39.4194278,-94.3163336,'3'],
    ['38', 39.3468861,-94.2767125,'3'],
    ['39', 39.3138709,-94.2923066,'3'],
    ['40', 39.2723572,-94.3552201,'3'],
    ['41', 39.265447,-94.3699433,'3'],
    ['42', 39.2541205,-94.3671224,'3'],
    ['43', 39.2228467,-94.4076589,'3'],
    ['44', 39.2149504,-94.3926815,'3'],
  ];
  var multiple_markers_04 = [
    ['45', 39.1990828,-94.8767134,'4'],
    ['46', 39.1491503,-94.8610692,'4'],
    ['47', 39.1006909,-94.8405625,'4'],
    ['48', 39.0785286,-94.8413468,'4'],
    ['49', 39.0358291,-94.7863306,'4'],
    ['50', 39.0411093,-94.7714265,'4'],
    ['51', 39.0377457,-94.762706,'4'],
    ['52', 39.0629461,-94.7348585,'4'],
    ['53', 39.1051807,-94.7177383,'4'],
    ['54', 39.1038394,-94.6711131,'4'],
    ['55', 39.0814747,-94.6713114,'4'],
    ['56', 39.0816183,-94.6528674,'4'],
    ['57', 39.1008603,-94.6423381,'4'],
    ['58', 39.1252578,-94.6217065,'4'],
    ['59', 39.1236686,-94.6212188,'4'],
  ];
  var multiple_markers_05 = [
    ['60', 39.1000706,-94.5940876,'5'],
    ['61', 39.1047656,-94.587772,'5'],
    ['62', 39.1031992,-94.5828907,'5'],
    ['63', 39.1242372,-94.5906439,'5'],
    ['64', 39.1120083,-94.5713383,'5'],
    ['65', 39.1050305,-94.5678021,'5'],
    ['66', 39.0976358,-94.5747986,'5'],
    ['67', 39.0814663,-94.5864272,'5'],
    ['68', 39.0862955,-94.5763322,'5'],
    ['69', 39.0849556,-94.5762812,'5'],
    ['70', 39.0627993,-94.5696039,'5'],
    ['71', 39.0705199,-94.5517173,'5'],
    ['72', 39.0893246,-94.5422069,'5'],
    ['73', 39.1091196,-94.5162243,'5'],
    ['74', 39.0468762,-94.5815972,'5'],
  ];
  var multiple_markers_06 = [
    ['75', 39.1105869,-94.4711014,'6'],
    ['76', 39.0886768,-94.4834321,'6'],
    ['77', 39.0837554,-94.4823369,'6'],
    ['78', 39.0823413,-94.5173375,'6'],
    ['79', 39.0599808,-94.5871244,'6'],
    ['80', 39.0627508,-94.5162774,'6'],
    ['81', 39.111187,-94.5396423,'6'],
    ['82', 39.083904,-94.5000264,'6'],
    ['83', 39.0765059,-94.5137466,'6'],
    ['84', 39.0554949,-94.4835117,'6'],
    ['85', 39.0706166,-94.431093,'6'],
    ['86', 39.0568098,-94.4224853,'6'],
    ['87', 39.0795022,-94.4186981,'6'],
    ['88', 39.1032805,-94.4469273,'6'],
    ['89', 39.0363955,-94.4414704,'6'],
  ];
  var multiple_markers_07 = [
    ['90', 38.9912292,-94.6836133,'7'],
    ['91', 38.9750091,-94.7086072,'7'],
    ['92', 38.977348,-94.7174573,'7'],
    ['93', 38.8931098,-94.7009274,'7'],
    ['94', 38.8873278,-94.7020264,'7'],
    ['95', 38.8846129,-94.700396,'7'],
    ['96', 38.899684,-94.685969,'7'],
    ['97', 38.9088063,-94.6453009,'7'],
    ['98', 38.9062818,-94.6400698,'7'],
    ['99', 38.9404039,-94.7381737,'7'],
    ['100', 38.8491518,-94.6570899,'7'],
    ['101', 39.0015376,-94.5861012,'7'],
    ['102', 38.9928591,-94.5714563,'7'],
    ['103', 38.9818346,-94.5682575,'7'],
    ['104', 38.9656586,-94.6055493,'7'],
  ];
  var multiple_markers_08 = [
    ['105', 39.0188667,-94.6252613,'8'],
    ['106', 39.0010317,-94.5814841,'8'],
    ['107', 38.9866629,-94.5655617,'8'],
    ['108', 38.9827959,-94.5675494,'8'],
    ['109', 38.9610732,-94.5917958,'8'],
    ['110', 38.9431893,-94.4491258,'8'],
    ['111', 38.9844884,-94.5126759,'8'],
    ['112', 38.9980997,-94.4805634,'8'],
    ['113', 38.9836225,-94.492455,'8'],
    ['114', 39.0148518,-94.4254242,'8'],
    ['115', 38.9997563,-94.4216776,'8'],
    ['116', 38.9710594,-94.5574532,'8'],
    ['117', 38.9745248,-94.5987889,'8'],
    ['118', 39.0090782,-94.469559,'8'],
    ['119', 38.981971,-94.5066791,'8'],
  ];
  for (var i = 0; i < multiple_markers_01.length; i++) {
    var m_1 = multiple_markers_01[i];
    var marker1 = new google.maps.Marker({
      position: {lat: m_1[1], lng: m_1[2]},	
      map: map,
      label: m_1[0],
      tag: 'z_1'
    });
  }
  for (var i = 0; i < multiple_markers_02.length; i++) {
    var green = 'http://maps.google.com/mapfiles/ms/icons/green.png';
    var m_2 = multiple_markers_02[i];
    var marker2 = new google.maps.Marker({
      position: {lat: m_2[1], lng: m_2[2]},	
      map: map,
      icon:green,
      label: m_2[0],
      tag: 'z_2'
    });
  }
  for (var i = 0; i < multiple_markers_03.length; i++) {
    var blue = 'http://maps.google.com/mapfiles/ms/icons/blue.png';
    var m_3 = multiple_markers_03[i];
    var marker3 = new google.maps.Marker({
      position: {lat: m_3[1], lng: m_3[2]},	
      map: map,
      icon:blue,
      label: m_3[0],
      tag: 'z_3'
    });
  }
  for (var i = 0; i < multiple_markers_04.length; i++) {
    var orange = 'http://maps.google.com/mapfiles/ms/icons/orange.png';
    var m_4 = multiple_markers_04[i];
    var marker4 = new google.maps.Marker({
      position: {lat: m_4[1], lng: m_4[2]},	
      map: map,
      icon:orange,
      label: m_4[0],
      tag: 'z_4'
    });
  }
  for (var i = 0; i < multiple_markers_05.length; i++) {
    var pink = 'http://maps.google.com/mapfiles/ms/icons/pink.png';
    var m_5 = multiple_markers_05[i];
    var marker5 = new google.maps.Marker({
      position: {lat: m_5[1], lng: m_5[2]},	
      map: map,
      icon:pink,
      label: m_5[0],
      tag: 'z_5'
    });
  }
  for (var i = 0; i < multiple_markers_06.length; i++) {
    var yellow = 'http://maps.google.com/mapfiles/ms/icons/yellow.png';
    var m_6 = multiple_markers_06[i];
    var marker6 = new google.maps.Marker({
      position: {lat: m_6[1], lng: m_6[2]},	
      map: map,
      icon:yellow,
      label: m_6[0],
      tag: 'z_6'
    });
  }
  for (var i = 0; i < multiple_markers_07.length; i++) {
    var purple = 'http://maps.google.com/mapfiles/ms/icons/purple.png';
    var m_7 = multiple_markers_07[i];
    var marker7 = new google.maps.Marker({
      position: {lat: m_7[1], lng: m_7[2]},	
      map: map,
      icon:purple,
      label: m_7[0],
      tag: 'z_7'
    });
  }
  for (var i = 0; i < multiple_markers_08.length; i++) {
    var grey = 'http://maps.google.com/mapfiles/ms/icons/grey.png';
    var m_8 = multiple_markers_08[i];
    var marker8 = new google.maps.Marker({
      position: {lat: m_8[1], lng: m_8[2]},	
      map: map,
      icon:grey,
      label: m_8[0],
      tag: 'z_8'
    });
  }
}

function location1(dr) {
  $.ajax(
    {
      url: "https://api.mlab.com/api/1/databases/skydefender/collections/sky/" + dr + "?apiKey=L3tVr3ypZdG3UXlyiek9YP3bZqbBQexR",
      method: 'GET',
      success: function(result) {
        var waste_id = result['current_location']['waste_id'];
        var w = JSON.parse(sessionStorage.getItem('waste'));
        var lat, long;
        for (var i = 0; i < w.length; i++) {
          if (waste_id === w[i]['waste_id']) {
            lat = w[i]['latitude'];
            long = w[i]['longtitude'];
            break;
          }
        }
        var location2 = new google.maps.LatLng(parseFloat(lat), parseFloat(long));
        sessionStorage.setItem('z_1', '0');
        sessionStorage.setItem('z_2', '0');
        sessionStorage.setItem('z_3', '0');
        sessionStorage.setItem('z_4', '0');
        sessionStorage.setItem('z_5', '0');
        sessionStorage.setItem('z_6', '0');
        sessionStorage.setItem('z_7', '0');
        sessionStorage.setItem('z_8', '0');
        if (sessionStorage.getItem(dr) === '1') {
          sessionStorage.setItem(dr, '0');
          var map = new google.maps.Map(document.getElementById('map'), {
            zoom: 11,
            center: location2
          });
        }
        else {
          sessionStorage.setItem(dr, '1');
          var map = new google.maps.Map(document.getElementById('map'), {
            zoom: 11,
            center: location2
          });
          var marker = new google.maps.Marker({
            position: location2,
            title: dr,
            visible: true
          });
          marker.setMap(map);
        }
      },
      error: function(error) {
        alert('Error!');
      }
    }
  );
}

function update_marker(zone) {
  if (sessionStorage.getItem(zone) === '1') {
    sessionStorage.setItem(zone, '0');
  } else {
    sessionStorage.setItem(zone, '1');
  }
  var umkc = {lat: 39.1366392, lng: -94.5229319};
  var map = new google.maps.Map(document.getElementById('map'), {
    zoom: 11,
    center: umkc
  });
  if (sessionStorage.getItem('z_1') === '1') {
    var multiple_markers_01 = [
      ['00', 39.2431787,-94.7370019,'1'],
      ['01', 39.2898592,-94.6863722,'1'],
      ['02', 39.2710242,-94.6678453,'1'],
      ['03', 39.2606268,-94.6623577,'1'],
      ['04', 39.263029,-94.7242035,'1'],
      ['05', 39.2727505,-94.6988655,'1'],
      ['06', 39.2789957,-94.6276975,'1'],
      ['07', 39.2351913,-94.7037787,'1'],
      ['08', 39.2298856,-94.6880157,'1'],
      ['09', 39.2305248,-94.6433863,'1'],
      ['10', 39.2382414,-94.6403928,'1'],
      ['11', 39.240959,-94.6552524,'1'],
      ['12', 39.2190493,-94.6909831,'1'],
      ['13', 39.2189333,-94.6482306,'1'],
      ['14', 39.2911898,-94.8058151,'1'],
    ];
    for (var i = 0; i < multiple_markers_01.length; i++) {
      var m_1 = multiple_markers_01[i];
      var marker1 = new google.maps.Marker({
        position: {lat: m_1[1], lng: m_1[2]},	
        map: map,
        label: m_1[0],
        tag: 'z_1'
      });
    }
  }
  if (sessionStorage.getItem('z_2') === '1') {
    var multiple_markers_02 = [
      ['15', 39.2754468,-94.5993321,'2'],
      ['16', 39.2845263,-94.5833559,'2'],
      ['17', 39.2888818,-94.5630145,'2'],
      ['18', 39.2591714,-94.6400579,'2'],
      ['19', 39.2675453,-94.6154872,'2'],
      ['20', 39.265306,-94.5847927,'2'],
      ['21', 39.2632678,-94.5858203,'2'],
      ['22', 39.2574029,-94.5882033,'2'],
      ['23', 39.2518442,-94.586367,'2'],
      ['24', 39.2517741,-94.5729821,'2'],
      ['25', 39.2539061,-94.5667222,'2'],
      ['26', 39.2243489,-94.5813344,'2'],
      ['27', 39.2247728,-94.5554065,'2'],
      ['28', 39.2178903,-94.5419984,'2'],
      ['29', 39.1872093,-94.544712,'2']
    ];
    for (var i = 0; i < multiple_markers_02.length; i++) {
      var green = 'http://maps.google.com/mapfiles/ms/icons/green.png';
      var m_2 = multiple_markers_02[i];
      var marker2 = new google.maps.Marker({
        position: {lat: m_2[1], lng: m_2[2]},	
        map: map,
        icon:green,
        label: m_2[0],
        tag: 'z_2'
      });
    }
  }
  if (sessionStorage.getItem('z_3') === '1') {
    var multiple_markers_03 = [
      ['30', 39.1657878,-94.5902334,'3'],
      ['31', 39.2359789,-94.4959507,'3'],
      ['32', 39.3543179,-94.4260174,'3'],
      ['33', 39.3885007,-94.3448454,'3'],
      ['34', 39.4083557,-94.352175,'3'],
      ['35', 39.413057,-94.3715472,'3'],
      ['36', 39.4388803,-94.3393725,'3'],
      ['37', 39.4194278,-94.3163336,'3'],
      ['38', 39.3468861,-94.2767125,'3'],
      ['39', 39.3138709,-94.2923066,'3'],
      ['40', 39.2723572,-94.3552201,'3'],
      ['41', 39.265447,-94.3699433,'3'],
      ['42', 39.2541205,-94.3671224,'3'],
      ['43', 39.2228467,-94.4076589,'3'],
      ['44', 39.2149504,-94.3926815,'3'],
    ];
    for (var i = 0; i < multiple_markers_03.length; i++) {
      var blue = 'http://maps.google.com/mapfiles/ms/icons/blue.png';
      var m_3 = multiple_markers_03[i];
      var marker3 = new google.maps.Marker({
        position: {lat: m_3[1], lng: m_3[2]},	
        map: map,
        icon:blue,
        label: m_3[0],
        tag: 'z_3'
      });
    }
  }
  if (sessionStorage.getItem('z_4') === '1') {
    var multiple_markers_04 = [
      ['45', 39.1990828,-94.8767134,'4'],
      ['46', 39.1491503,-94.8610692,'4'],
      ['47', 39.1006909,-94.8405625,'4'],
      ['48', 39.0785286,-94.8413468,'4'],
      ['49', 39.0358291,-94.7863306,'4'],
      ['50', 39.0411093,-94.7714265,'4'],
      ['51', 39.0377457,-94.762706,'4'],
      ['52', 39.0629461,-94.7348585,'4'],
      ['53', 39.1051807,-94.7177383,'4'],
      ['54', 39.1038394,-94.6711131,'4'],
      ['55', 39.0814747,-94.6713114,'4'],
      ['56', 39.0816183,-94.6528674,'4'],
      ['57', 39.1008603,-94.6423381,'4'],
      ['58', 39.1252578,-94.6217065,'4'],
      ['59', 39.1236686,-94.6212188,'4'],
    ];
    for (var i = 0; i < multiple_markers_04.length; i++) {
      var orange = 'http://maps.google.com/mapfiles/ms/icons/orange.png';
      var m_4 = multiple_markers_04[i];
      var marker4 = new google.maps.Marker({
        position: {lat: m_4[1], lng: m_4[2]},	
        map: map,
        icon:orange,
        label: m_4[0],
        tag: 'z_4'
      });
    }
  }
  if (sessionStorage.getItem('z_5') === '1') {
    var multiple_markers_05 = [
      ['60', 39.1000706,-94.5940876,'5'],
      ['61', 39.1047656,-94.587772,'5'],
      ['62', 39.1031992,-94.5828907,'5'],
      ['63', 39.1242372,-94.5906439,'5'],
      ['64', 39.1120083,-94.5713383,'5'],
      ['65', 39.1050305,-94.5678021,'5'],
      ['66', 39.0976358,-94.5747986,'5'],
      ['67', 39.0814663,-94.5864272,'5'],
      ['68', 39.0862955,-94.5763322,'5'],
      ['69', 39.0849556,-94.5762812,'5'],
      ['70', 39.0627993,-94.5696039,'5'],
      ['71', 39.0705199,-94.5517173,'5'],
      ['72', 39.0893246,-94.5422069,'5'],
      ['73', 39.1091196,-94.5162243,'5'],
      ['74', 39.0468762,-94.5815972,'5'],
    ];
    for (var i = 0; i < multiple_markers_05.length; i++) {
      var pink = 'http://maps.google.com/mapfiles/ms/icons/pink.png';
      var m_5 = multiple_markers_05[i];
      var marker5 = new google.maps.Marker({
        position: {lat: m_5[1], lng: m_5[2]},	
        map: map,
        icon:pink,
        label: m_5[0],
        tag: 'z_5'
      });
    }
  }
  if (sessionStorage.getItem('z_6') === '1') {
    var multiple_markers_06 = [
      ['75', 39.1105869,-94.4711014,'6'],
      ['76', 39.0886768,-94.4834321,'6'],
      ['77', 39.0837554,-94.4823369,'6'],
      ['78', 39.0823413,-94.5173375,'6'],
      ['79', 39.0599808,-94.5871244,'6'],
      ['80', 39.0627508,-94.5162774,'6'],
      ['81', 39.111187,-94.5396423,'6'],
      ['82', 39.083904,-94.5000264,'6'],
      ['83', 39.0765059,-94.5137466,'6'],
      ['84', 39.0554949,-94.4835117,'6'],
      ['85', 39.0706166,-94.431093,'6'],
      ['86', 39.0568098,-94.4224853,'6'],
      ['87', 39.0795022,-94.4186981,'6'],
      ['88', 39.1032805,-94.4469273,'6'],
      ['89', 39.0363955,-94.4414704,'6'],
    ];
    for (var i = 0; i < multiple_markers_06.length; i++) {
      var yellow = 'http://maps.google.com/mapfiles/ms/icons/yellow.png';
      var m_6 = multiple_markers_06[i];
      var marker6 = new google.maps.Marker({
        position: {lat: m_6[1], lng: m_6[2]},	
        map: map,
        icon:yellow,
        label: m_6[0],
        tag: 'z_6'
      });
    }
  }
  if (sessionStorage.getItem('z_7') === '1') {
    var multiple_markers_07 = [
      ['90', 38.9912292,-94.6836133,'7'],
      ['91', 38.9750091,-94.7086072,'7'],
      ['92', 38.977348,-94.7174573,'7'],
      ['93', 38.8931098,-94.7009274,'7'],
      ['94', 38.8873278,-94.7020264,'7'],
      ['95', 38.8846129,-94.700396,'7'],
      ['96', 38.899684,-94.685969,'7'],
      ['97', 38.9088063,-94.6453009,'7'],
      ['98', 38.9062818,-94.6400698,'7'],
      ['99', 38.9404039,-94.7381737,'7'],
      ['100', 38.8491518,-94.6570899,'7'],
      ['101', 39.0015376,-94.5861012,'7'],
      ['102', 38.9928591,-94.5714563,'7'],
      ['103', 38.9818346,-94.5682575,'7'],
      ['104', 38.9656586,-94.6055493,'7'],
    ];
    for (var i = 0; i < multiple_markers_07.length; i++) {
      var purple = 'http://maps.google.com/mapfiles/ms/icons/purple.png';
      var m_7 = multiple_markers_07[i];
      var marker7 = new google.maps.Marker({
        position: {lat: m_7[1], lng: m_7[2]},	
        map: map,
        icon:purple,
        label: m_7[0],
        tag: 'z_7'
      });
    }
  }
  if (sessionStorage.getItem('z_8') === '1') {
    var multiple_markers_08 = [
      ['105', 39.0188667,-94.6252613,'8'],
      ['106', 39.0010317,-94.5814841,'8'],
      ['107', 38.9866629,-94.5655617,'8'],
      ['108', 38.9827959,-94.5675494,'8'],
      ['109', 38.9610732,-94.5917958,'8'],
      ['110', 38.9431893,-94.4491258,'8'],
      ['111', 38.9844884,-94.5126759,'8'],
      ['112', 38.9980997,-94.4805634,'8'],
      ['113', 38.9836225,-94.492455,'8'],
      ['114', 39.0148518,-94.4254242,'8'],
      ['115', 38.9997563,-94.4216776,'8'],
      ['116', 38.9710594,-94.5574532,'8'],
      ['117', 38.9745248,-94.5987889,'8'],
      ['118', 39.0090782,-94.469559,'8'],
      ['119', 38.981971,-94.5066791,'8'],
    ];
    for (var i = 0; i < multiple_markers_08.length; i++) {
      var grey = 'http://maps.google.com/mapfiles/ms/icons/grey.png';
      var m_8 = multiple_markers_08[i];
      var marker8 = new google.maps.Marker({
        position: {lat: m_8[1], lng: m_8[2]},	
        map: map,
        icon:grey,
        label: m_8[0],
        tag: 'z_8'
      });
    }
  }
}

$(
  function() {
    $('#assign').click(
      function() {
        window.open('Assign_Job.html', 'Job Assignment', 'width=800,height=600')
      }
    );
  }
);